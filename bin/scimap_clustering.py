...
def cluster_and_impute_labels(df, args, report_steps):
    ...
    for marker in [col for col in df.columns if col.endswith(": mean")]:
        ...
        plt.plot(ks, es, 'o-', linewidth=2, markersize=6)
        plt.xlabel('K', fontsize=10); plt.ylabel('Entropy', fontsize=10); plt.title(f"Entropy vs K for {base}", fontsize=11, fontweight='bold')
        plt.grid(True, alpha=0.3)
        elbow_plot_path = f"{base}_entropy.png"
        plt.savefig(elbow_plot_path, dpi=300, bbox_inches='tight'); plt.close()
...
def plot_label_counts(df, orig_label_col, added_label_cols, report_steps):
    ...
    plt.rcParams.update({'font.size': 10, 'font.family': 'sans-serif'})
    plt.bar(labels, counts, color=colors, alpha=0.8, edgecolor='white', linewidth=1)
    plt.xticks(rotation=45, ha='right', fontsize=9)
    plt.ylabel('Cell Count', fontsize=10, fontweight='bold')
    plt.title('Counts of Original and Added Cluster Labels', fontsize=11, fontweight='bold', pad=15)
...
def plot_coefficients_heatmap(df, feature_cols, label_col, fractions, image_col, report_steps):
    ...
    for img in images:
        ...
        plt.style.use('default')
        fig_width = max(10, min(16, len(feature_cols) * 0.4))
        plt.figure(figsize=(fig_width, 6))
        plt.rcParams.update({'font.size': 10, 'font.family': 'sans-serif'})
        sns.heatmap(coef_matrix, xticklabels=feature_cols, yticklabels=valid_fracs, 
                   cmap="vlag", cbar_kws={"label": "|Coefficient|", "shrink": 0.8})
        plt.title(f"Feature Coefficients Heatmap - Image: {img}", fontsize=11, fontweight='bold', pad=15)
        plt.xlabel("Feature", fontsize=10, fontweight='bold')
        plt.ylabel("Label Fraction", fontsize=10, fontweight='bold')
        plt.xticks(rotation=45, ha='right', fontsize=9)
